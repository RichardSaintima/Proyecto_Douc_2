...
form.addEventListener('submit', function(evt){
    ...
    if(nombre.value.length < 3 || nombre.value.length > 20){
        document.getElementById('nombreError').innerHTML = "El nombre debe tener al menos 3 y no más de 20 caracteres";
    }
    ...
    if(apellido.value.length < 3 || apellido.value.length > 20){
        document.getElementById('apellidoError').innerHTML = "El Apellido paterno debe tener al menos 3 y no más de 20 caracteres";
    }
    ...
    if(apellido2.value.length < 3 || apellido2.value.length > 20){
        document.getElementById('apellido2Error').innerHTML = "El Apellido materno debe tener al menos 3 y no más de 20 caracteres";
    }

    if(telefono.value.length < 9 || telefono.value.length > 12){
        document.getElementById('telefonoError').innerHTML = "N° telefono no válido";
    }

    if(email.value === null || email.value === ''){
        document.getElementById('emailError').innerHTML = "Debes ingresar un Correo";
    }

    if(password.value === null || password.value === ''){
        document.getElementById('passwordError').innerHTML = "Debes ingresar tu contraseña";
    }
    ...
    if (password.value !== confirmPassword.value){
        document.getElementById('passwordError2').innerHTML = "Las contraseñas son diferentes";
    }
